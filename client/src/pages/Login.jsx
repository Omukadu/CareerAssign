  ...
  return (
    <div className="min-h-screen grid place-items-center bg-gradient-to-br from-brand-50 to-white p-6">
      <form onSubmit={submit} className="card w-full max-w-md p-8">
        ...
        <input type="password" className="input mb-6" value={password} onChange={e => setPassword(e.target.value)} required />
        <button disabled={loading} className="btn-primary w-full">{loading ? 'Logging in…' : 'Login'}</button>
        <p className="text-sm text-gray-500 text-center mt-4">No account? <Link to="/register" className="text-brand-700 font-medium">Register</Link></p>
      </form>
    </div>
  );
}
