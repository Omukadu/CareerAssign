...
export default function CareerDetail() {
  ...
  useEffect(() => {
    ...
    api
      .get("/saved")
      .then((r) => setSaved(r.data.some((s) => s.career._id === id)))
      .catch(() => {});
  }, [id]);

  const toggleSave = async () => {
    try {
      if (saved) await api.delete(`/saved/${id}`);
      else await api.post(`/saved/${id}`);
      setSaved(!saved);
    } catch {}
  };
  const del = async () => {
  ...
  return (
    <div className="max-w-4xl">
      <Link
        to="/careers"
        className="inline-flex items-center text-sm text-gray-500 mb-4"
      >
        ...
        <div className="p-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex gap-2">
              ...
              <Link to={`/careers/${c._id}/edit`} className="btn-outline">
                <Pencil size={16} />
              </Link>
              <button onClick={del} className="btn-outline text-red-600">
                <Trash2 size={16} />
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-6">
            <Stat
              label="Avg. Salary"
              value={`$${c.avgSalary?.toLocaleString()}`}
            />
          ...
          {c.skills?.length > 0 && (
            <div className="mt-6">
              ...
              <div className="flex flex-wrap gap-2">
                {c.skills.map((s) => (
                  <span key={s._id} className="chip bg-brand-50 text-brand-700">
                    {s.name}
                  </span>
                ))}
              </div>
